...
function CanvasDivider(){
    ...
    this.slice = function( currShape, alternate, delay ){
        ...
        if(alternate){
            TweenLite.to(bmp, 1, {delay: delay, x: bounds.x + 100, y:bounds.y - 180, /*rotation: -20,*/ onUpdate:__stage.update, onUpdateScope:__stage, ease: Expo.easeInOut} );
        }else{
            TweenLite.to(bmp, 1, {delay: delay, x: bounds.x - 100, y:bounds.y + 180, /*rotation: 20,*/ onUpdate:__stage.update, onUpdateScope:__stage, ease: Expo.easeInOut} );
        }
...
function init(){
    ...
    for(var i = 0; i < shapes.length; i++){
        canvasDivider.slice(i, (i%2==0), Math.random() * (shapes.length * .05));
    }
